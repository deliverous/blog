...
Les triggers ou webhook sont là pour déclancher à distance le déploiement de votre projet. Comme votre projet peut regrouper plusieurs sources de données (nombreux dépôt git) Il est possible de créer plusieurs trigger par exemple pour séparer les données 
