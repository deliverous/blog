...
Pour définir un conteneur il suffit de le nomer en début de ligne, par exemple pour un conteneur que l'on appelle "front" : 
...
Certaines clés demandent simplement une valeur, d'autres, une liste de valeures, voir même une liste de sous clés et de valeurs. Par exemple : 
...
Défini la commande à executer. Cette valeur est ajoutée à la fin de la commande `docker run`
...
Défini une restriction de déploiement, permet de s'assurer que 2 conteneurs seront déployés sur des machines physique différentes.
...
Défini une affinité de déploiement, permet de s'assurer que 2 conteneurs seront déployés sur la même machine physique.
...
Défini un serveur DNS spécifique pour le conteneur. Correspond à l'option `--dns` de `docker run`
...
Dans cet exemple, le conteneur sera démarré avec `docker run --dns=127.0.0.1 deliverous/blog`. Cette option en plus de l'installation d'un serveur dnsmasq à l'intérieur du conteneur permet entre autre de résoudre les problèmes de modification du fichier `/etc/hosts` par le conteneur lui même.
...
Permet de définir des variables d'environements par une liste de clé valeur.
...
Défini le nom réseau du conteneur, correspond à l'option `--hostname` de la ligne de commande
...
Dans cet exemple le conteneur `front` demande à etre lié au conteneur mysql qu'il nommera `db`.
...
Permet de sécifier l'adresse source qui sera utilisé pour toutes les connections sortante du conteneur. Comme pour l'option `ports` il est possible de définir l'ip soit par son nom, soit par l'ip elle même.
